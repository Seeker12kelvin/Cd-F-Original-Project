...
const PropertyList = ({properties, sor}) => {
  return (
    <div aria-label="Property results" className={`flex w-full h-full overflow-scroll ${!sor ? 'flex-col gap-4': 'justify-around flex-wrap'}`}>
      {sor ?
        properties.map(data => {
        return(
        ...
        properties.map(data => (
          <Link key={data.id} to={`/rent/${data.id}`}>
            <article className={`${styles['onLoad-animation']} w-full h-40 flex gap-7 rounded-lg border-[#F0EFFB] border-[0.09375rem]`}>
              <figure className='w-[35%] h-full object-cover rounded-l-lg'>
                <img className='h-full rounded-l-lg' src={data.imgUrl} alt={`A picture of ${data.name}`} />
              </figure>

              <section className='self-center w-[65%]'>
